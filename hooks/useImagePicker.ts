...
const useImagePicker = () => {
  const resize = async (uri: string, base64: string, w: number, h: number) => {
    if (w > MAX_SIZE || h > MAX_SIZE) {
      ...
      const manipResult = await ImageManipulator.manipulateAsync(uri, [{ resize }], {
        base64: true,
      });
  ...
  const takePhoto = async (callback?: ImagePickerCallBack, aspect?: [number, number]) => {
    if (Platform.OS !== 'web') {
    ...
    if (result && result.canceled) {
      ...
      callback &&
        callback(
          await resize(
            result.assets[0].uri,
            result.assets[0].base64 ? result.assets[0].base64 : '',
